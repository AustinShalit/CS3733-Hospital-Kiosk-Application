...
import edu.wpi.cs3733.d19.teamO.util.Debouncer;
...
public class ExtendedPropertySheet extends PropertySheet {
	...
	public static class PropertyItem<T> implements Item {
		 ...
		 * the name of the property by converting from <tt>camelCase</tt> to a natural-language
		 * <tt>Sentence case</tt> text. For example, a property with the name <tt>"fooBarBaz"</tt> will
		 * generate a name of <tt>"Foo bar baz"</tt>. If a name other than the property name is desired,
		 * or if generating a sentence-case string would be inappropriate, use {@link
		 ...
		 * (<tt>' '</tt>) and that character's lower-case representation.
		 */
